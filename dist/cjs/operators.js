...
const customOperators = new Map(
  Object.entries({
    ...
    $query: (element, args) => {
      ...
      try {
        let results = (0, import_queryElements.queryElements)({ element, selector });
    ...
    $getObjectValue: (element, args) => {
      if (Array.isArray(args) && args.length >= 2) return (0, import_getValueFromObject.getValueFromObject)(args[0], args[1]);
      return "";
    },
...
function resolveOperator(element, operator, args, parent, params, objectRegistry) {
  ...
  if (args && typeof args === "string") {
    ...
    if (objMatch && objectRegistry.has(args)) {
      ...
      let qResults = (0, import_queryElements.queryElements)({ element, selector: args });
      if (!qResults) {
        targetElements = [];
      } else {
